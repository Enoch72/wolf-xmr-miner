...
__kernel void cn1(__global uint4 *Scratchpad, __global ulong *states, int variant, __global ulong *input)
{
	...
	for(int i = 0; i < 0x80000; ++i)
	{
		...
		((uint4 *)c)[0] = Scratchpad[IDX((((uint)a[0]) & 0x1FFFF0) >> 4)];
		...
		Scratchpad[IDX((((uint)a[0]) & 0x1FFFF0) >> 4)] = b_x;
		
		uint4 tmp;
		tmp = Scratchpad[IDX((((uint)c[0]) & 0x1FFFF0) >> 4)];
		
		a[1] += c[0] * as_ulong2(tmp).s0;
		a[0] += mul_hi(c[0], as_ulong2(tmp).s0);

		VARIANT1_2(a[1]);
		Scratchpad[IDX((((uint)c[0]) & 0x1FFFF0) >> 4)] = ((uint4 *)a)[0];
		VARIANT1_2(a[1]);
